...
class MMapArea (Gtk.DrawingArea):
    ...
    def button_down (self, widget, event):
        ...
        if event.button == 2:
            ...
            self.get_window().set_cursor (Gdk.CursorType.FLEUR)
    ...
    def realize_cb (self, widget):
        ...
        if self.mode == MODE_IMAGE or self.mode == MODE_DRAW:
            self.get_window().set_cursor (Gdk.CursorType.CROSSHAIR)
        else:
            self.get_window().set_cursor (Gdk.CursorType.LEFT_PTR)
        return False
    ...
    def set_mouse_cursor_cb (self, thought, cursor_type):
        if not self.moving:
            self.get_window().set_cursor (cursor_type)
    ...
    def expose (self, widget, event):
        ...
        #print(help(self))
        context = self.get_window().cairo_create()#self.window.cairo_create ()
    ...
    def draw (self, event, context):
        ...
        #print(event.rectangle)
        area = event.path_extents()
        print(area)
        context.rectangle (area[0], area[1], area[2], area[3])
        context.clip ()
        context.set_source_rgb (1.0,1.0,1.0)
        context.move_to (area[0], area[1])
        context.paint ()
        ...
        ax, ay = self.transform_coords(area[0], area[1])
        width  = area[2] / self.scale_fac
        height = area[3] / self.scale_fac
        print(ax)
        print(ay)
        print(width)
        print(height)
    ...
    def create_new_thought (self, coords, thought_type = None, loading = False):
        ...
        if type == TYPE_TEXT:
            ...
            thought = ImageThought.ImageThought (coords, self.Pango_context, self.nthoughts, self.save, self.undo)
        elif type == TYPE_DRAWING:
            thought = DrawingThought.DrawingThought (coords, self.Pango_context, self.nthoughts, self.save, self.undo, loading)
        elif type == TYPE_RESOURCE:
            thought = ResourceThought.ResourceThought (coords, self.Pango_context, self.nthoughts, self.save, self.undo, loading)
        if not thought.okay ():
            return None
